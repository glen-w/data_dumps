...
def test_browser_and_ring_available_and_fetch(browser_ring_conn):
    ...
    assert set(monthly["series_id"]) <= {"browser_urls_last_seen", "ring_events"}
    assert "year_month" in monthly.columns

    long_df = crq.fetch_panel(
        browser_ring_conn,
        ["browser_urls_last_seen", "ring_events"],
        grain="daily",
    )
    assert not long_df.empty
    assert set(long_df["metric_id"]) <= {"browser_urls_last_seen", "ring_events"}
