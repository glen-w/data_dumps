...
- Load IPs, phones, payment instruments, KYC / ads / inference blobs, precise home addresses, or Wi‑Fi SSIDs **by default**. Drop those at ingest; keep a forbidden-column test. Do **not** add email columns to source tables. The Tools tab (`email_inventory.py`) is the only listing of addresses — yours, other people's, and ones only mentioned in text. When a new text column can contain an address, append a `TextScan` there. Do **not** add IP columns either. The same tab (`ip_inventory.py`) lists login, session, and access-log addresses from the original export. When a new dump has a dropped IP file, add an extractor there. Message text is OK when the product is messaging (Telegram, Slack, LinkedIn, Twitter) — document it.
- Invent a **dynamic** plugin registry (entry points, auto-import of every module, scanning a plugins directory). Built-in dumps append to the thin explicit [`CONTRIBUTIONS`](../../src/data_dumps/contributions.py) list. A person adding their own export without a repo change uses the custom manifest or the single [`user_contributions.py`](services/custom.md) file under the data root — that file is imported by path, not discovered.
