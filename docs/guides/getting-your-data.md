...
Wall-clock columns use a hardcoded zone, not the machine timezone: `Europe/Rome` for most sources, `Europe/Paris` for Slack, Google, Airbnb, ChatGPT, Cursor History, and Ollama, `Europe/London` for Ring.
...
| [Ollama](services/ollama.md) | Local app database (not a download) | `~/Library/Application Support/Ollama/db.sqlite` | Ollama | Chat text kept; attachment bytes, account email, and device id dropped |
